 ...
 *
 * Root `playwright.config.ts` already declares a project named "project" with the
 * right testDir and baseURL. This file is what gets loaded when you run
 * `pnpm test --config=products/project/playwright.config.ts` or when you target
 * just this product through `--project=project`.
 *
...
export default defineConfig({
  ...
  testMatch: /.*\.spec\.ts$/,
  outputDir: 'test-results/project',
  timeout: 30_000,
